...
// Displays a welcome message, a notification bell with a
// dropdown inbox, and a profile menu with logout and settings.
//
// Key behaviours:
//  - Clicking outside the dropdowns closes them automatically
//  - Unread notification count shown as an animated red badge
//  - Profile menu shows the user's name, role/grade, and email
// ─────────────────────────────────────────────────────────
...
const Topbar = () => {
  ...
  // Refs used to detect clicks outside the dropdowns
  const profileRef = useRef(null);
  const notificationRef = useRef(null);

  // Count how many notifications are still unread for the badge
  ...
  // Attach a global mousedown listener to close dropdowns when
  // the user clicks anywhere outside of them
  useEffect(() => {
        ...
        <div className="relative" ref={notificationRef}>
          ...
          {showNotifications && (
            <div className="absolute right-0 mt-3 w-80 bg-white rounded-2xl shadow-2xl border border-gray-100 overflow-hidden transform origin-top-right transition-all animate-in fade-in slide-in-from-top-2 duration-200">
              {/* Dropdown header with mark-all-read and clear buttons */}
              ...
              <div className="max-h-[400px] overflow-y-auto">
                {notifications?.length === 0 ? (
                  ...
                  // One row per notification — clicking marks it as read
                  notifications?.map((n) => (
                    <div 
                      key={n.id} 
                    ...
                    >
                      {/* Blue dot for unread, invisible for read */}
                      ...
                      <div className="flex-grow">
                        ...
                        <span className="text-[10px] text-gray-400 font-medium">
                          {/* Format the timestamp as "Jan 15, 02:30 PM" */}
        ...
        <div className="relative" ref={profileRef}>
          ...
          >
            ...
            <div className="flex flex-col text-right hidden sm:flex">
              ...
              {/* Show grade+section for students, role for others */}
          ...
          {showProfileMenu && (
            <div className="absolute right-0 mt-3 w-56 bg-white rounded-2xl shadow-2xl border border-gray-100 overflow-hidden py-2 transform origin-top-right transition-all animate-in fade-in slide-in-from-top-2 duration-200">
              ...
              {/* Link to the user's settings page */}
              <Link 
